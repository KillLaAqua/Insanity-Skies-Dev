...
ServerEvents.recipes((event) => {
    ...
    ['blackstone', 'calcite', 'tuff', 'dripstone_block'].forEach((stone) => {
        event.recipes.gtceu
            ...
            .adjacentFluids(
                'minecraft:lava',
                stone == 'blackstone' ? 'exnihilosequentia:witch_water' : 'minecraft:water'
            )
            .duration(16)
            .EUt(7);
    });
    ...
    plungerMats.forEach((material) => {
        event
            .shaped(Item.of(`gtceu:${material}_plunger`), ['WPP', ' SP', 'S F'], {
    ...
    malletMats.forEach((material) => {
        event
            .shaped(Item.of(`gtceu:${material}_mallet`), ['II ', 'IIS', 'II '], {
    ...
    event.recipes.gtceu
        .electrolyzer(id('sodalite_decomposition'))
    ...
    event.recipes.gtceu
        .circuit_assembler(id('data_dna_disk'))
        .itemInputs(
    ...
    event.recipes.gtceu
        ...
        .itemInputs(
            'kubejs:abyssal_printed_circuit_board',
            '2x #gtceu:circuits/uiv',
            '18x kubejs:stellar_ram_chip',
            '48x kubejs:hyper_nor_memory_chip',
            '48x kubejs:hyper_nand_memory_chip',
    ...
    ['drum', 'fluid_cell'].forEach((container) => {
        ['enriched_naquadah', 'neutronium'].forEach((type) => {
            event.shapeless(Item.of(`start_core:${type}_${container}`), [Item.of(`start_core:${type}_${container}`)]);
        });
    });

    //NPK Re-add
    event.recipes.gtceu
        .large_chemical_reactor(id('npk_solution'))
    ...
    event.recipes.gtceu
        .electrolyzer(id('npk_solution_decomposition'))
    ...
    event
        .shapeless(Item.of('3x minecraft:red_mushroom'), ['minecraft:red_mushroom_block', '#forge:tools/mortars'])
        .id('start:shapeless/red_mushroom');
    event.recipes.gtceu
        .macerator(id('red_mushrooms'))
    ...
    event.recipes.gtceu
        .alloy_blast_smelter(id('naquadated_soldering_alloy'))
        .itemInputs(
            '3x gtceu:tin_dust',
            '18x gtceu:indium_dust',
            '6x gtceu:silver_dust',
            '4x gtceu:lutetium_dust',
            '3x gtceu:cerium_dust',
            '3x gtceu:naquadah_dust',
            '1x gtceu:trinium_dust',
            '2x gtceu:lead_dust'
        )
    ...
    event.recipes.gtceu
        .alloy_blast_smelter(id('neutrindium_soldering_alloy'))
        .itemInputs(
            '2x gtceu:neutronium_dust',
            '51x gtceu:indium_dust',
            '6x gtceu:tin_dust',
            '4x gtceu:darmstadtium_dust',
            '5x gtceu:cadmium_dust',
            '4x gtceu:enriched_naquadah_dust',
    ...
    event.recipes.gtceu
        .centrifuge(id('impure_naquadria_solution_separation'))
        .inputFluids('gtceu:impure_naquadria_solution 4000')
        .itemOutputs(
            '2x gtceu:indium_phosphide_dust',
            '1x gtceu:trinium_sulfide_dust',
